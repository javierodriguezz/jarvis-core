...
def grabar_audio(
    duracion_segundos: float = 4.0, cuenta_regresiva: bool = True
) -> np.ndarray:
    """
    Graba audio del micrófono por defecto durante duracion_segundos.

    Con cuenta_regresiva=True muestra un conteo antes de empezar, para darle
    tiempo al usuario de prepararse -- si no, se pierde el inicio de lo que
    dice. Se puede apagar con cuenta_regresiva=False cuando el usuario ya está
    hablando y esperar sería contraproducente: es el caso del modo de palabra
    de activación (Fase 7), donde acaba de decir "hey Jarvis" y sigue de largo.
